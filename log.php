...
if ($_REQUEST['clear']){
    if (!$fileName) die;
    if ($_REQUEST['clear'] == 'all' or $_REQUEST['clear'] >= filesize($fileName)) {
...
class ArrayLog {
    ...
    public function getNextLine(){
        do{
            if (!$this->nextLine){
                $curLine = fgets($this->file);
            }else{
                $curLine = $this->nextLine;
            }
            $this->nextLine = fgets($this->file);

        }while(strstr($curLine, '(3) applying pattern')
                and strstr($this->nextLine, '(3) applying pattern'));
        return $curLine;
    }
...
<div class="tools">
    <form method="GET" action="<?= $_SERVER['REQUEST_URI'] ?>" >
        <input type="text" name="path" id="path" placeholder="path to your log file" value="<?=$fileName?>" />
